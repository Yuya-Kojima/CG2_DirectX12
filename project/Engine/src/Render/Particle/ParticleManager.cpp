...
void ParticleManager::Update(const Matrix4x4& viewMatrix,
	...
	for (auto& [name, group] : particleGroups_) {
		...
		for (auto it = group.particles.begin(); it != group.particles.end();) {
			...
			if (instanceIndex < kNumMaxInstance_) {
				...
				Matrix4x4 worldViewProjectionMatrix =
					Multiply(Multiply(worldMatrix, viewMatrix_), projectionMatrix_);
				...
				group.instancingData[instanceIndex].color = c;
		...
		// 描画に載せる数だけをnumInstanceにする
		group.numInstance = instanceIndex;
	}
}
...
void ParticleManager::Draw(const std::string& name) {
	...
	commandList->SetPipelineState(graphicsPipeLineState_.Get()); // PSOを設定
	...
	commandList->DrawInstanced(group.vertexCount, group.numInstance, 0, 0);
	// }
}
...
void ParticleManager::Emit(const ParticleEmitDesc& desc) {

	if (!HasGroup(desc.name)) {
		Logger::Log(std::format("Emit: group not found: {}\n", desc.name));
		return;
	}

	ParticleGroup& group = particleGroups_.at(desc.name);
	...
	for (uint32_t i = 0; i < desc.count; ++i) {
		...
		particle.transform.scale.x =
			(std::max)(0.0001f, particle.transform.scale.x);
		particle.transform.scale.y =
			(std::max)(0.0001f, particle.transform.scale.y);
		particle.transform.scale.z =
			(std::max)(0.0001f, particle.transform.scale.z);
...
void ParticleManager::Emit(const std::string& name, const Vector3& position,
	...
	float lifeMax, const Vector4& color,
	const Vector3& baseScale, const Vector3& scaleRandom,
	const Vector3& baseRotate,
	const Vector3& rotateRandom) {
...
void ParticleManager::CreateVertexData(ParticleGroup& group) {
	...
	// VertexResourceを作る
	//=========================
	group.vertexResource = dx12Core_->CreateBufferResource(sizeof(VertexData) * 6);
	...
	group.vertexBufferView.SizeInBytes = sizeof(VertexData) * 6;
...
void ParticleManager::CreateParticleGroup(const std::string& name,
	...
	// インスタンシング用リソースの生成
	particleGroup.instancingResource = dx12Core_->CreateBufferResource(
	...
	srvManager_->CreateSRVforStructuredBuffer(
		particleGroup.instancingSrvIndex, particleGroup.instancingResource.Get(),
		kNumMaxInstance_, sizeof(ParticleForGPU));

	CreateVertexData(particleGroup);
}

void ParticleManager::CreateRingParticleGroup(const std::string& name,
	const std::string& textureFilePath,
	uint32_t divide,
	float outerRadius,
	float innerRadius) {
	...
	const auto& verts = ring.GetVertices();

	group.vertexCount = (uint32_t)verts.size();
...
bool ParticleManager::HasGroup(const std::string& name) const {
	return particleGroups_.find(name) != particleGroups_.end();
}
