...
void Bloom::Initialize(Dx12Core* dx12Core, SrvManager* srvManager, uint32_t width, uint32_t height) {
  width_ = width;
  height_ = height;
...
void Bloom::CreateExtractPipeline(Dx12Core* dx12Core) {
  ...
  hr = dx12Core->GetDevice()->CreateRootSignature(0, signatureBlob->GetBufferPointer(), signatureBlob->GetBufferSize(), IID_PPV_ARGS(&extractRootSignature_));
  assert(SUCCEEDED(hr));

  IDxcBlob* vsBlob = dx12Core->CompileShader(L"resources/shaders/Fullscreen.VS.hlsl", L"vs_6_0");
  IDxcBlob* psBlob = dx12Core->CompileShader(L"resources/shaders/LuminanceExtract.PS.hlsl", L"ps_6_0");
  ...
  hr = dx12Core->GetDevice()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&extractPipelineState_));
  assert(SUCCEEDED(hr));
}
...
void Bloom::CreateBlurPipeline(Dx12Core* dx12Core) {
  ...
  hr = dx12Core->GetDevice()->CreateRootSignature(0, signatureBlob->GetBufferPointer(), signatureBlob->GetBufferSize(), IID_PPV_ARGS(&blurRootSignature_));
  assert(SUCCEEDED(hr));

  IDxcBlob* vsBlob = dx12Core->CompileShader(L"resources/shaders/Fullscreen.VS.hlsl", L"vs_6_0");
  IDxcBlob* psBlob = dx12Core->CompileShader(L"resources/shaders/GaussianBlur.PS.hlsl", L"ps_6_0");
  ...
  hr = dx12Core->GetDevice()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&blurPipelineState_));
  assert(SUCCEEDED(hr));
}

uint32_t Bloom::Draw(Dx12Core* dx12Core, SrvManager* srvManager, uint32_t srcSrvIndex) {
  ...
  commandList->SetGraphicsRootConstantBufferView(0, extractConstBuffer_->GetGPUVirtualAddress());
  ...
  commandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
  commandList->SetGraphicsRootConstantBufferView(0, blurConstBuffer_->GetGPUVirtualAddress());
  srvManager->SetGraphicsRootDescriptorTable(1, luminanceTexture_->GetSrvIndex());
