...
void Object3dRenderer::CreateRootSignature() {
  ...
  descriptorRangeEnvMap[0].OffsetInDescriptorsFromTableStart =
      D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

  rootParameterObject3d[7].ParameterType =
      D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
  rootParameterObject3d[7].ShaderVisibility =
      D3D12_SHADER_VISIBILITY_PIXEL;
  rootParameterObject3d[7].DescriptorTable.pDescriptorRanges =
...
void Object3dRenderer::BeginSkinning() {
  Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList> commandList =
      dx12Core_->GetCommandList();

  // Skinning用のRootSignatureをセット
...
void Object3dRenderer::CreateSkinningRootSignature() {
  HRESULT hr;
  ID3D12Device *device = dx12Core_->GetDevice();
  ...
  // 0~7はそのままコピー
  rootParameterObject3d[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
  rootParameterObject3d[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
  rootParameterObject3d[0].Descriptor.ShaderRegister = 0;

  rootParameterObject3d[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
  rootParameterObject3d[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;
  ...
  rootParameterObject3d[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
  rootParameterObject3d[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
  rootParameterObject3d[2].DescriptorTable.pDescriptorRanges = descriptorRange;
  rootParameterObject3d[2].DescriptorTable.NumDescriptorRanges = _countof(descriptorRange);

  rootParameterObject3d[3].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
  rootParameterObject3d[3].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
  rootParameterObject3d[3].Descriptor.ShaderRegister = 1;

  rootParameterObject3d[4].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
  rootParameterObject3d[4].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
  rootParameterObject3d[4].Descriptor.ShaderRegister = 2;

  rootParameterObject3d[5].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
  rootParameterObject3d[5].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
  rootParameterObject3d[5].Descriptor.ShaderRegister = 3;

  rootParameterObject3d[6].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
  rootParameterObject3d[6].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
  rootParameterObject3d[6].Descriptor.ShaderRegister = 4;
  ...
  rootParameterObject3d[7].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
  rootParameterObject3d[7].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
  rootParameterObject3d[7].DescriptorTable.pDescriptorRanges = descriptorRangeEnvMap;
  rootParameterObject3d[7].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeEnvMap);

  // [8] 新規追加：SkinningのMatrixPalette用(t0, VertexShader)
  D3D12_DESCRIPTOR_RANGE descriptorRangeMatrixPalette[1] = {};
  descriptorRangeMatrixPalette[0].BaseShaderRegister = 0; // t0
  descriptorRangeMatrixPalette[0].NumDescriptors = 1;
  descriptorRangeMatrixPalette[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
  descriptorRangeMatrixPalette[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

  rootParameterObject3d[8].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
  rootParameterObject3d[8].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX; // VSで使う
  rootParameterObject3d[8].DescriptorTable.pDescriptorRanges = descriptorRangeMatrixPalette;
  rootParameterObject3d[8].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeMatrixPalette);
  ...
  descriptionRootSignature.pParameters = rootParameterObject3d;
  descriptionRootSignature.NumParameters = _countof(rootParameterObject3d);
  descriptionRootSignature.pStaticSamplers = staticSamplers;
  descriptionRootSignature.NumStaticSamplers = _countof(staticSamplers);
  ...
  skinningRootSignature_ = nullptr;
...
void Object3dRenderer::CreateSkinningPSO() {
  HRESULT hr;
  ID3D12Device *device = dx12Core_->GetDevice();
  ...
  inputElementDescs[3].InputSlot = 1; // 1番目のslotのVBV
  inputElementDescs[3].AlignedByteOffset = D3D12_APPEND_ALIGNED_ELEMENT;

  inputElementDescs[4].SemanticName = "INDEX";
  inputElementDescs[4].SemanticIndex = 0;
  inputElementDescs[4].Format = DXGI_FORMAT_R32G32B32A32_SINT; // SINT
  inputElementDescs[4].InputSlot = 1; // 1番目のslotのVBV
  ...
  IDxcBlob *vertexShaderBlob = dx12Core_->CompileShader(L"resources/shaders/SkinningObject3d.VS.hlsl", L"vs_6_0");
  assert(vertexShaderBlob != nullptr);

  IDxcBlob *geometryShaderBlob = dx12Core_->CompileShader(L"resources/shaders/Object3d.GS.hlsl", L"gs_6_0");
  assert(geometryShaderBlob != nullptr);

  IDxcBlob *pixelShaderBlob = dx12Core_->CompileShader(L"resources/shaders/Object3d.PS.hlsl", L"ps_6_0");
  assert(pixelShaderBlob != nullptr);
  ...
  D3D12_GRAPHICS_PIPELINE_STATE_DESC graphicsPipeLineStateDesc{};
  graphicsPipeLineStateDesc.pRootSignature = skinningRootSignature_.Get();
  ...
  skinningGraphicsPipeLineState_ = nullptr;
  hr = device->CreateGraphicsPipelineState(&graphicsPipeLineStateDesc, IID_PPV_ARGS(&skinningGraphicsPipeLineState_));
  assert(SUCCEEDED(hr));
}
